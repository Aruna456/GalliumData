...
public class ResponseLoggerFilter extends DNSResponseFilter
{
    ...
    public void configure(final FilterUse def) {
        ...
        if (baseFilename != null && baseFilename.trim().length() > 0) {
            ...
            try {
                ...
                ResponseLoggerFilter.log.error(Markers.DNS, "Unable to open log file for requests: " + filename + ", reason: " + ex.getMessage());
                this.out = null;
            }
        }
        else {
    ...
    public String getName() {
        return ResponseLoggerFilter.class.getName();
    }
