...
public class ResponseLoggerFilter implements ResponseFilter
{
    ...
    public void configure(final FilterUse def) {
        ...
        if (baseFilename != null && baseFilename.trim().length() > 0) {
            ...
            try {
                ...
                ResponseLoggerFilter.log.error(Markers.POSTGRES, "Unable to open log file for requests: " + filename + ", reason: " + ex.getMessage());
                this.out = null;
            }
        }
        else {
    ...
    private void logText(final String s, final Variables context) {
        ...
        if (formatStr.contains("$ts")) {
            final String nowStr = ZonedDateTime.now().format(ResponseLoggerFilter.logTsFormat);
        ...
        if (formatStr.contains("$ip")) {
            final Variables connCtxt = (Variables)context.get("connectionContext");
            String ip = (String)connCtxt.get("userIP");
            if (ip == null) {
                ip = "<unknown>";
            }
