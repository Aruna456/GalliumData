...
public class TokenDoneInProc extends MessageToken
{
    ...
    public TokenDoneInProc(final ConnectionState connectionState) {
    ...
    public int readFromBytes(final byte[] bytes, final int offset, final int numBytes) {
        ...
        this.statusSrvError = ((status & 0x20) > 0);
        this.curCmd = DataTypeReader.readTwoByteIntegerLow(bytes, idx);
        idx += 2;
        if (this.connectionState.tdsVersion72andHigher()) {
            this.rowCount = DataTypeReader.readEightByteInteger(bytes, idx);
            idx += 8;
        }
        else {
            this.rowCount = DataTypeReader.readFourByteInteger(bytes, idx);
            idx += 4;
        }
        return idx - offset;
    }
    ...
    public void write(final RawPacketWriter writer) {
        ...
        if (this.connectionState.tdsVersion72andHigher()) {
            ...
            writer.writeFourByteInteger((int)this.rowCount);
        }
    }
    ...
    public String toString() {
        return "Done in proc, ID " + this.curCmd + ", #rows " + this.rowCount + (this.statusError ? " - error" : "");
    }
