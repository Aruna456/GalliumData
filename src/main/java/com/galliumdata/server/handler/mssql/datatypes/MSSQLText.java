...
public class MSSQLText extends MSSQLDataType
{
    private byte[] dummyTxt;
    
    public MSSQLText(final ColumnMetadata meta) {
        super(meta);
    }
    ...
    public int readFromBytes(final byte[] bytes, final int offset) {
        ...
        if (dummyTxtPtrSize == 0) {
            this.value = MSSQLText.NULL_VALUE;
            return 1;
        }
        ...
        idx += 4;
        this.value = new String(bytes, idx, textSize, StandardCharsets.UTF_8);
        idx += textSize;
        return idx - offset;
    }
    ...
    public void read(final RawPacketReader reader) {
        ...
        this.value = reader.readStringWithEncoding(textSize, this.meta.getTypeInfo().getCollationLCID());
    }
    
    @Override
    public int getSerializedSize() {
        if (this.value == MSSQLText.NULL_VALUE) {
            return 1;
        }
        int size = 1;
        size += 16;
    ...
    public String toString() {
        if (this.value == null || this.value == MSSQLText.NULL_VALUE) {
            return "text (null)";
        }
